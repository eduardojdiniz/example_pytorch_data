...
from utils import IMG_EXTENSIONS, DATA_DIR, default_image_loader, is_valid_extension
...
def make_dataset(
    ...
    is_dir_or_symlink = bool(directory.is_dir() or directory.is_symlink())
    assert is_dir_or_symlink, f"{directory} is not a valid directory"
    ...
    if extensions is not None:
        ...
        def _is_valid_file(fname: Path) -> bool:
            return is_valid_extension(fname, cast(Tuple[str, ...], extensions))

    is_valid_file = cast(Callable[[Path], bool], _is_valid_file)
    ...
    for target_class in sorted(class_to_idx.keys()):
        ...
        class_instances = []
        n_instances = 0
        target_dir = pjoin(directory, target_class)
        if not os.path.isdir(target_dir):
            continue
        for root, _, fnames in sorted(os.walk(target_dir, followlinks=True)):
            for fname in sorted(fnames):
                if is_valid_file(Path(fname)):
